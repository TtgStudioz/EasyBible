...
async function fetchVerse(verse, version) {
    ...
    const proxyUrl = `https://odds-olympics-stock-frame.trycloudflare.com/scrape?url=${encodeURIComponent(targetUrl)}&selector=.passage-content`;
...
function lightTheme(activated) {
    if (activated) {
        ...
        document.getElementById("holder").style.borderWidth = "1px";
        document.getElementById("holder").style.borderColor = "black";
        document.getElementById("holder").style.borderStyle = "solid";
        document.getElementById("languages").style.backgroundColor = "#447d9e"
        document.getElementById("dropdownContent").style.backgroundColor = "#447d9e"
        // document.querySelectorAll(".dropdown-content")[0].style.scrollbarColor = " #b5b5b5 #447d9e";

        document.querySelectorAll(".dropbtn").forEach(element => {
            element.style.backgroundColor = "#447d9e"
        })
        ...
        document.getElementById("holder").style.borderWidth = "0px";
        document.body.style.scrollbarColor = "#fdfdfd #1e1e1e"
        document.getElementById("languages").style.backgroundColor = "#2f5a73"
        document.getElementById("dropdownContent").style.backgroundColor = "#2f5a73"

        document.querySelectorAll(".dropbtn").forEach(element => {
            element.style.backgroundColor =  "#2f5a73"
        })
...
function copyToClipboard(text) {
    // Check if the clipboard API is available.  It should be available in
    // modern browsers.  If it isn't, the function will reject.
    if (!navigator.clipboard) {
      return Promise.reject(new Error("Clipboard API is not available in this browser."));
    }
  
    // Use the writeText method of the clipboard API to copy the text.
    return navigator.clipboard.writeText(text);
  }


document.getElementById("copy").addEventListener("click", function() {
    copyToClipboard(document.getElementById("verseText").innerText)
    var tooltip = document.getElementById("myTooltip");
    tooltip.innerHTML = "Copied";
})

document.getElementById("verseInput").addEventListener("focus", function() {
    document.getElementById("holder").style.boxShadow = "rgb(0 0 0 / 0%) 0px 0px 0px 0px, rgb(58 151 212 / 36%) 0px 0px 10px 4px, rgb(0 0 0 / 0%) 0px 0px 0px 0px, rgb(60 66 87 / 16%) 0px 0px 0px 1px, rgb(0 0 0 / 0%) 0px 0px 0px 0px, rgb(0 0 0 / 0%) 0px 0px 0px 0px, rgb(0 0 0 / 0%) 0px 0px 0px 0px";
})

document.getElementById("verseInput").addEventListener("blur", function() {
    document.getElementById("holder").style.boxShadow = "";
})
